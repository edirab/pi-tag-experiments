...
unsigned long FiducialModelPi::GetPoints(cv::Mat& image, std::vector<t_points>& vec_points)
{
	...
	m_image_size_factor = image.cols * 1. / 1920.0; 
...
// ------------ Ellipse extraction --------------------------------------
    ...
    for(size_t i = 0; i < contours.size(); i++)
    {
		...
		if (box_max > std::min(src_mat_8U1.rows, src_mat_8U1.cols) * 0.05)
            continue;
        //if (box_min < 0.5*min_ellipse_size)
		if (box_min < 0.9 * min_ellipse_size)
            continue;
        ...
        if(m_use_fast_pi_tag)
        {
            ...
            if((box.size.area() > 200  || box.size.area() < 50) && (ellipse_aspect_ratio < 0.75 || ellipse_aspect_ratio > 1.6))
                continue;
        ...
        if (debug)
        {
            ...
            for (unsigned int i=0; i<final_tag_vec.size(); i++)
            {
                ...
                for (unsigned int j=0; j<final_tag_vec[i].image_points.size(); j++)
                {
                    ...
                    {
                        cv::circle(tag_image, final_tag_vec[i].image_points[j].center, 3, cv::Scalar(0,255,0), -1, cv::LINE_AA);
                        if (connect_points)
                        {
                            cv::line(tag_image, final_tag_vec[i].image_points[j-1].center, final_tag_vec[i].image_points[j].center, cv::Scalar(rgbVal[0], rgbVal[1], rgbVal[2]), 1, cv::LINE_AA);
                        }
                        connect_points = true;
                    }
                    else
...
unsigned long FiducialModelPi::LoadParameters(std::string directory_and_filename)
{
        ...
        std::shared_ptr<TiXmlDocument> p_configXmlDocument (new TiXmlDocument( directory_and_filename.c_str() ));
...
//************************************************************************************
                    ...
                    if( p_xmlElement_fpitag ) {
                        ...
                        if ( p_xmlElement_fpitag->QueryBoolAttribute( "value", &m_use_fast_pi_tag) != TIXML_SUCCESS)
                        {
                            std::cerr << "ERROR - FiducialModelPi::LoadParameters:" << std::endl;
...
//************************************************************************************
                                ...
                                {
                                        // read and save value of attribute
                                        if ( p_xmlElement_Child->QueryIntAttribute( "value", &pi_parameters.m_id) != TIXML_SUCCESS)
                                        {
                                                std::cerr << "ERROR - FiducialModelPi::LoadParameters:" << std::endl;
...
//************************************************************************************
                                ...
                                {
                                        // read and save value of attribute
                                        if ( p_xmlElement_Child->QueryDoubleAttribute( "value", &pi_parameters.line_width_height) != TIXML_SUCCESS)
                                        {
                                                std::cerr << "ERROR - FiducialModelPi::LoadParameters:" << std::endl;
...
//************************************************************************************
                                ...
                                {
                                        // read and save value of attribute
                                        if ( p_xmlElement_Child->QueryDoubleAttribute( "AB", &pi_parameters.d_line0_AB) != TIXML_SUCCESS)
                                        {
                                                std::cerr << "ERROR - FiducialModelPi::LoadParameters:" << std::endl;
                                        ...
                                        // read and save value of attribute
                                        if ( p_xmlElement_Child->QueryDoubleAttribute( "AC", &pi_parameters.d_line0_AC) != TIXML_SUCCESS)
                                        {
                                                std::cerr << "ERROR - FiducialModelPi::LoadParameters:" << std::endl;
...
//************************************************************************************
                                ...
                                {
                                        // read and save value of attribute
                                        if ( p_xmlElement_Child->QueryDoubleAttribute( "AB", &pi_parameters.d_line1_AB) != TIXML_SUCCESS)
                                        {
                                                std::cerr << "ERROR - FiducialModelPi::LoadParameters:" << std::endl;
                                        ...
                                        // read and save value of attribute
                                        if ( p_xmlElement_Child->QueryDoubleAttribute( "AC", &pi_parameters.d_line1_AC) != TIXML_SUCCESS)
                                        {
                                                std::cerr << "ERROR - FiducialModelPi::LoadParameters:" << std::endl;
...
//************************************************************************************
                                ...
                                {
                                        // read and save value of attribute
                                        if ( p_xmlElement_Child->QueryDoubleAttribute( "x", &pi_parameters.m_offset.x) != TIXML_SUCCESS)
                                        {
                                                std::cerr << "ERROR - FiducialModelPi::LoadParameters:" << std::endl;
                                        ...
                                        // read and save value of attribute
                                        if ( p_xmlElement_Child->QueryDoubleAttribute( "y", &pi_parameters.m_offset.y) != TIXML_SUCCESS)
                                        {
                                                std::cerr << "ERROR - FiducialModelPi::LoadParameters:" << std::endl;
...
//************************************************************************************
                                ...
                                {
                                        // read and save value of attribute
                                        if ( p_xmlElement_Child->QueryDoubleAttribute( "x", &m_general_fiducial_parameters[pi_parameters.m_id].m_sharpness_pattern_area_rect3d.x) != TIXML_SUCCESS)
                                        {
                                                std::cerr << "ERROR - FiducialModelPi::LoadParameters:" << std::endl;
                                        ...
                                        // read and save value of attribute
                                        if ( p_xmlElement_Child->QueryDoubleAttribute( "y", &m_general_fiducial_parameters[pi_parameters.m_id].m_sharpness_pattern_area_rect3d.y) != TIXML_SUCCESS)
                                        {
                                                std::cerr << "ERROR - FiducialModelPi::LoadParameters:" << std::endl;
                                        ...
                                        // read and save value of attribute
                                        if ( p_xmlElement_Child->QueryDoubleAttribute( "width", &m_general_fiducial_parameters[pi_parameters.m_id].m_sharpness_pattern_area_rect3d.width) != TIXML_SUCCESS)
                                        {
                                                std::cerr << "ERROR - FiducialModelPi::LoadParameters:" << std::endl;
                                        ...
                                        // read and save value of attribute
                                        if ( p_xmlElement_Child->QueryDoubleAttribute( "height", &m_general_fiducial_parameters[pi_parameters.m_id].m_sharpness_pattern_area_rect3d.height) != TIXML_SUCCESS)
                                        {
                                                std::cerr << "ERROR - FiducialModelPi::LoadParameters:" << std::endl;
